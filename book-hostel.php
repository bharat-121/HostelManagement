...
$stmt->execute();
echo"<script>alert('Student Successfully Allotted Room');</script>";
}
?>
...
$uid=$_SESSION['login'];
				...
				{ 
					$roomalreadybooked=1;?>
			<marquee><h1 style="color: lime">Room has already been booked by you.</h1></marquee>
				<?php }
				else{
					$roomalreadybooked=0;
					?>
					<marquee><h1 style="color: lime">Please Book your Room.</h1></marquee>
						<?php	}			
							?>
<?php	

if($roomalreadybooked==0)
{ ?>
...
<div class="col-sm-8">
<input type="text" name="seater" id="seater"  class="form-control" style="text-align:left" >
</div>
</div>
...
$(document).ready(function() {
	$('#duration').keyup(function(){
		...
		success:function(data){
	    $('.result').val(data); // to calculate total amount
		}
		});
